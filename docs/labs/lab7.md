...
o	In the "Assistant name" field, enter a meaningful name for your assistant.
o	Example: Fund Management Bot (or any other relevant name based on your use case).
o	The name is used internally and is not visible to end users.
2.	Provide a Description (Optional)
o	You can add a short description that summarizes the purpose of this assistant.
o	Example: "A chatbot to assist users with fund management queries and investment options."
o	This step is optional but helps in organizing multiple assistants.
3.	Select Assistant Language
o	From the "Assistant language" dropdown, choose the preferred language for the assistant.
o	By default, English (U.S.) is selected.
4.	Proceed to the Next Step
o	Click the "Next" button at the top-right corner to continue to the Personalization step.
 
...
o	Under "Tell us where your assistant will live", select where you plan to deploy the assistant.
o	Options may include Web, Mobile App, Messaging Platforms, or Others.
o	Example: Select "Web" if you plan to use the assistant on a website.
2.	Provide Industry Information
o	In the "Which industry do you work in?" dropdown, select your industry.
o	Example: Banking and Financial Services (if the assistant is for a finance-related use case).
3.	Define Your Role
o	In "What is your role on the team building the assistant?", choose the most relevant role.
o	Example: Designer, Developer, Product Manager, etc.
4.	Select a Need Statement
o	Choose a statement that best describes your AI assistant’s purpose.
o	Example: "I want to make it easier for my customers to find what they’re looking for in my app."
o	This helps refine the assistant’s focus and experience.
...
o	Under "Assistant’s name as known by customers", enter the name that users will see when they interact with the assistant.
o	Example: "Fund Management Assistant"
2.	Choose the Intended Purpose
o	Select an option based on the assistant’s main use case:
...
o	Recommended: Standard for most business use cases.
3.	Select a Theme
o	Choose between Light or Dark mode based on your brand style.
o	Example: Light mode for a clean, professional look.
4.	Customize Colors
o	Primary Color (Chat Header): Define the main color of the assistant's interface.
o	Secondary Color (User Message Bubble): Choose a contrasting color for messages sent by users.
o	Account Color (Special AI Actions): Assign a color to highlight AI-driven responses.
o	Example:
	Primary: #FFFFFF (White)
	Secondary: #303D1D (Dark Green)
	Account: #0345AE (Blue)
5.	Set Chat Widget Size (Optional)
o	Adjust the width and height of the chatbot window.
o	Default: Width – 380px, Height – 640px
o	Leave as is if unsure.
6.	IBM Watermark
o	Enable or Disable the "Built with IBM WatsonX" watermark based on preference.
7.	Enable Streaming (Optional)
o	Streaming allows messages to appear dynamically as they are being typed.
o	Recommended: Keep ON for a more natural user experience.
8.	Upload an Avatar Image (Optional)
o	Click "Add an avatar image" to upload a custom icon for your assistant.
...
o	The right panel displays a simulated website with the chatbot embedded.
o	Interact with the assistant by typing a message in the input field to see how it responds.
o	Verify that the UI elements, colors, and avatar (if uploaded) appear correctly.
2.	Customize Web Chat (Optional)
o	Click "Customize web chat" to modify the chatbot’s appearance further.
o	You can adjust its layout, colors, and positioning based on your website’s design.
3.	Share the Preview (Optional)
o	Click "Copy link to share" to generate a preview link that you can send to stakeholders for feedback.
...
In the Assistant says field, type “We have expert team of financial advisors, who will work with you to structure your financial portfolio.
We would encourage you to register with us.”
...
After you build an assistant, you can test to make sure it works as you intended before you make it available to customers. AI assistant builder makes it straightforward to preview your assistant in a closed environment and manage exactly what you make available to customers.
